...
public class ExpandAdapter_DatVe extends BaseExpandableListAdapter {
	...
	public View getGroupView(int groupPosition, boolean isExpanded,
		...
		//gan giá trị cho textview hiển thị node cha.
	...
	public View getChildView(final int groupPosition, final int childPosition,
		...
		int dem =0;
		//lấy danh sách các child node theo mỗi parent node.
		for(int i = 0; i < listlichchieu.size(); i++){
			if(listlichchieu.get(i).getTenrap().equals(item[groupPosition])){
				dem++;
			}
		...
		int k =0;
		//lấy thông tin các node từ danh sách child node đã lọc phía trên.
		for(int i = 0; i < listlichchieu.size(); i++){
			if(listlichchieu.get(i).getTenrap().equals(item[groupPosition])){
		...
		//gán settext cho mỗi child node dựa vào các thông đã lấy phía trên.
		btnitem.setText(noidung[childPosition]);
		//bắt sự kiện khi click vào mỗi item và gọi Class đặt vé để hiển thị sơ đồ rạp.
